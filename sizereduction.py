def main(X, Y):
    ...
    #This only works for averaging by 10, converts pixels to microns
    for x in range(len(condensedArray)):
        xValues.append(((x+1)*10 + 5.5)*8.87437e-6)
    for y in range(len(condensedArray[0])):
        yValues.append(((y+1)*10 + 5.5)*8.87437e-6)
    ...
    with open("interFile.txt",'w') as f: 
        f.write(str(condensedArray))

    #Writes each file
    with open("XValues.txt",'w') as f: 
        for x in xValues: 
            f.write(str(x) + '\n')
    with open("YValues.txt",'w') as f: 
        for y in yValues: 
            f.write(str(y) + '\n')    
    with open("ZMatrix.txt",'w') as f: 
        for z in zMatrix: 
            for z2 in z: 
